...
`src/render/loaders.ts` versucht nacheinander `room-refined.glb`, `room-redesign.glb` und `room.glb`. `src/render/room-assets.ts` startet Modell, erwarteten Lichtatlas und optionale LUT parallel. Der tatsächlich verwendete Atlas richtet sich weiterhin nach den Metadaten des geladenen Modells. Falls der neue Atlas nicht geladen werden kann, verwirft die Runtime das verfeinerte Modell und lädt die bisherige Raumvariante mit ihrem zugehörigen Atlas. Die alte PNG wird dadurch nicht auf neu gepackte UV1-Koordinaten gelegt. Falls nur die LUT fehlt, bleibt die vorhandene AgX-Näherung aktiv.
